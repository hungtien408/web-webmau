<%@ Page Title="" Language="C#" MasterPageFile="~/ad/template/adminEn.master" AutoEventWireup="true"
    CodeFile="careercategory.aspx.cs" Inherits="ad_single_careercategory" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBody" runat="Server">
        ...
        <asp:RadGrid ID="RadGrid1" AllowMultiRowSelection="True" runat="server" Culture="vi-VN" 
            ...
            <MasterTableView CommandItemDisplay="TopAndBottom" DataSourceID="ObjectDataSource1"
                ...
                <Columns>
                    ...
                    <asp:GridTemplateColumn DataField="CareerCategoryName" HeaderText="Tên danh mục">
                        <ItemTemplate>
                            <div class='<%#"catlevel level" +  Eval("Level") %>' style='padding-left: <%# string.IsNullOrEmpty(Eval("Level").ToString()) ? 0 : Convert.ToInt32(Eval("Level")) * 10 %>px'>
                                <asp:Label ID="lblCareerCategoryName" runat="server" Font-Bold='<%# Eval("ParentID").ToString() == "0" ? true : false %>'
                                    Text='<%# Eval("CareerCategoryName")%>'></asp:Label>
                            </div>
                        </ItemTemplate>
                    </asp:GridTemplateColumn>
                    <asp:GridTemplateColumn DataField="CareerCategoryNameEn" HeaderText="Tên danh mục(Tiếng Anh)">
                        <ItemTemplate>
                            <div class='<%#"catlevel level" +  Eval("Level") %>' style='padding-left: <%# string.IsNullOrEmpty(Eval("Level").ToString()) ? 0 : Convert.ToInt32(Eval("Level")) * 10 %>px'>
                                <asp:Label ID="lblCareerCategoryNameEn" runat="server" Font-Bold='<%# Eval("ParentID").ToString() == "0" ? true : false %>'
                                    Text='<%# Eval("CareerCategoryNameEn")%>'></asp:Label>
                            </div>
                        </ItemTemplate>
                    </asp:GridTemplateColumn>
                    <asp:GridBoundColumn DataField="CareerCategoryID" HeaderText="ID" SortExpression="CareerCategoryID">
                    </asp:GridBoundColumn>
                    <asp:GridTemplateColumn>
                        <ItemTemplate>
                            <div style="line-height: 10px; padding: 0 10px 0 10px;">
                                <asp:LinkButton ID="lnkUpOrder" runat="server" OnClick="lnkUpOrder_Click" rel='<%# Eval("CareerCategoryID") %>'>
                                    <img alt="" title="Lên 1 cấp" src="../assets/images/up-arrow.gif" />
                                </asp:LinkButton>
                                <asp:LinkButton ID="lnkDownOrder" runat="server" OnClick="lnkDownOrder_Click" rel='<%# Eval("CareerCategoryID") %>'>
                        ...
                        <ItemTemplate>
                            <asp:Panel ID="Panel1" runat="server" Visible='<%# string.IsNullOrEmpty( Eval("ImageName").ToString()) ? false : true %>'>
                                ...
                                <asp:LinkButton ID="lnkDeleteImage" runat="server" CommandName="DeleteImage" OnClientClick="return confirm('Xóa ảnh này ?')"
                                    rel='<%#  Eval("CareerCategoryID") + "#" + Eval("ImageName") %>'>
                        ...
                        <asp:Panel ID="Panel1" runat="server" DefaultButton="lnkUpdate">
                            ...
                            <table class="search">
                                <tr>
                                    ...
                                    <td>
                                        <asp:HiddenField ID="hdnCareerCategoryID" runat="server" Value='<%# Eval("CareerCategoryID") %>' />
                                        <asp:HiddenField ID="hdnImageName" runat="server" Value='<%# Eval("ImageName") %>' />
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <asp:RadComboBox ID="ddlParent" runat="server" DataSourceID='<%# (Container is GridEditFormInsertItem) ? "ObjectDataSource1" : "ObjectDataSource2" %>'
                                            DataTextField="CareerCategoryName" DataValueField="CareerCategoryID" Filter="Contains"
                                            OnDataBound="DropDownList_DataBound" Width="504px">
                                        </asp:RadComboBox>
                                        <asp:ObjectDataSource ID="ObjectDataSource2" runat="server" SelectMethod="CareerCategoryForEditSelectAll"
                                            TypeName="TLLib.CareerCategory">
                                            <SelectParameters>
                                                <asp:ControlParameter ControlID="hdnCareerCategoryID" Name="CareerCategoryID" PropertyName="Value"
                                                    Type="String" />
                                            </SelectParameters>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <asp:RadTextBox ID="txtCareerCategoryName" runat="server" Text='<%# (Container is GridEditFormInsertItem) ? "" : (string.IsNullOrEmpty(Eval("CareerCategoryName").ToString()) ? "" : Eval("CareerCategoryName").ToString().Remove(0, Convert.ToInt32(Eval("Level")))) %>'
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <asp:RadTextBox ID="txtCareerCategoryNameEn" runat="server" Text='<%# (Container is GridEditFormInsertItem) ? "" : Eval("CareerCategoryNameEn") %>'
    ...
    <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" DeleteMethod="CareerCategoryDelete"
        SelectMethod="CareerCategorySelectAll" TypeName="TLLib.CareerCategory" UpdateMethod="CareerCategoryUpdate"
        InsertMethod="CareerCategoryInsert">
        <DeleteParameters>
            <asp:Parameter Name="CareerCategoryID" Type="String" />
        </DeleteParameters>
        <InsertParameters>
            <asp:Parameter Name="CareerCategoryName" Type="String" />
            <asp:Parameter Name="CareerCategoryNameEn" Type="String" />
            <asp:Parameter Name="ConvertedCareerCategoryName" Type="String" />
